...
def load_data():
  ...
  train_data_dir = '../../../data/crop/outputs'
  validation_data_dir = '../../../data/crop/val_split'
...
def plot(history): 
    ...
    fig1 = plt.gcf()
    plt.show()
    fig1.savefig('graph/inception_v3_v5_finetune_accuracy.png', dpi=1000)
    ...
    fig2 = plt.gcf()
    plt.show()
    fig2.savefig('graph/inception_v3_v5_finetune_loss.png', dpi=1000)
     
def save_model_to_json(model):
    print('saving model...')
    with open('model\inception_v3_v5.json', 'w') as f:
        f.write(model.to_json())
    print('model saved...')    
    
def save_history_json(history):
    with open('history/inception_v3_v5_history.json', mode='w') as f:
...
if __name__ == '__main__':
    ...
    num_class = 6 
    batch_size = 16 
    nb_train_samples = 1068
    nb_validation_samples = 605
    nb_epoch = 100
    
    FishNames = ['ALB', 'BET', 'DOL', 'LAG', 'SHARK', 'YFT']
    ...
    filepath="weight/inception_v3_v5_finetune.best.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True,
    mode='max')
    
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=10, verbose=1, min_lr=0.0000001) 
    
    csv_logger = CSVLogger('history/inception_v3_v5_history.log', append=True)
