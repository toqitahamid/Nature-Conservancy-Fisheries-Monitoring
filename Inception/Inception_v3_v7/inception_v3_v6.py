...
def load_data():
  ...
  train_data_dir = '../../../data/299x299/preprocessed_train_split'
  validation_data_dir = '../../../data/299x299/preprocessed_val_split'
...
def plot(history): 
    ...
    fig1 = plt.gcf()
    plt.show()
    fig1.savefig('graph/inception_v3_v6_finetune_accuracy.png', dpi=1000)
    ...
    fig2 = plt.gcf()
    plt.show()
    fig2.savefig('graph/inception_v3_v6_finetune_loss.png', dpi=1000)
     
def save_model_to_json(model):
    print('saving model...')
    with open('model\inception_v3_v6.json', 'w') as f:
        f.write(model.to_json())
    print('model saved...')    
    
def save_history_json(history):
    with open('history/inception_v3_v6_history.json', mode='w') as f:
...
if __name__ == '__main__':
    ...
    num_class = 8 
    batch_size = 16 
    nb_train_samples = 3008
    nb_validation_samples = 756
    nb_epoch = 100
    ...
    filepath="weight/inception_v3_v6_finetune.best.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True,
    mode='max')
    
    reduce_lr = ReduceLROnPlateau(monitor='val_acc', factor=0.5, patience=5, verbose=1, min_lr=0.0000001) 
    
    csv_logger = CSVLogger('history/inception_v3_v6_history.log', append=True)
