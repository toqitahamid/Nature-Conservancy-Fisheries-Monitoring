...
def save_model_to_json(model):
    print('saving model...')
    with open('model\inception_v3_v1.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    filepath="weight/inception_v3_v1_finetune.best.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True,
    mode='max')
    
    reduce_lr = ReduceLROnPlateau(monitor='val_acc', factor=0.5, patience=10, verbose=1, min_lr=0.0000001) 
    
    csv_logger = CSVLogger('history/inception_v3_v1_history.log', append=True)
    finetune_callbacks_list = [checkpoint, csv_logger, reduce_lr] 
	
    from aetros.keras import KerasIntegration
    KerasIntegration('prithi/nature1', model, '453271d278866e6b1bd4455c4a302e6a', insights=True, confusion_matrix=True)	
