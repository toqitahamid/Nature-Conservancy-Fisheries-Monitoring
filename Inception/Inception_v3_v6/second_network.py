...
def inception_v3_model(img_rows, img_cols, channel=1, num_class=None):
    ...
    model.load_weights('weight/inception_v3_v6_finetune.best.hdf5')
...
def plot(history): 
    ...
    fig1 = plt.gcf()
    plt.show()
    fig1.savefig('graph/second_network_finetune_accuracy.png', dpi=1000)
    ...
    fig2 = plt.gcf()
    plt.show()
    fig2.savefig('graph/second_network_finetune_loss.png', dpi=1000)
     
def save_model_to_json(model):
    print('saving model...')
    with open('model\second_network.json', 'w') as f:
        f.write(model.to_json())
    print('model saved...')    
    
def save_history_json(history):
    with open('history/second_network_history.json', mode='w') as f:
...
if __name__ == '__main__':
    ...
    filepath="weight/second_network_finetune.best.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True,
    mode='max')
    
    reduce_lr = ReduceLROnPlateau(monitor='val_acc', factor=0.5, patience=5, verbose=1, min_lr=0.0000001) 
    
    csv_logger = CSVLogger('history/second_network_history.log', append=True)
